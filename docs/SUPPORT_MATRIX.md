...
`구현`은 코드·자동 테스트·문서가 저장소에 있다는 뜻이며 운영 검증 완료를 뜻하지 않습니다. `읽기 smoke`와 `거래 smoke`가 모두 `구현`이어야 실제 계정과 지정 송신 경로를 이용한 운영 검증까지 끝난 상태입니다.
...
| P4 | Crypto.com | Spot | 구현 | 구현 | 구현 | 구현 | 구현 | 예정 | 예정 | [문서](exchanges/CRYPTOCOM.md) |
