...
구현 언어는 **Go**로 확정했다. 다음 세 가지는 Phase 0에서 결정해야 한다.

1. **“지원”의 첫 범위**: P0에서 시세+현물 주문까지만 먼저 낼지, Binance/Bitget 선물까지 같은 릴리스에 포함할지 결정한다.
2. **EIP 선택 정책**: 요청자가 항상 route를 지정할지, 계정 기본 route를 허용할지 결정한다. 제안은 “계정 기본값 + 요청별 재정의”다.
3. **배포 환경**: EC2 OS, instance type, 필요한 EIP 개수, IaC 도구, Secret 저장소를 확정한다.
