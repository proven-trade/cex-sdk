// Package bithumb은 빗썸 Spot REST 어댑터를 제공한다.
package bithumb
...
func (decimal *Decimal) UnmarshalJSON(data []byte) error {
	...
	if len(trimmed) == 0 {
		return fmt.Errorf("Bithumb decimal value is empty")
	}
	if trimmed[0] == '"' {
		var value string
...
// OrderReference는 v2 주문 생성 응답의 주문 식별 정보다.
type OrderReference struct {
	...
	STPType       string          `json:"stp_type"`
	Raw           json.RawMessage `json:"-"`
}

// CancelResult는 v2 주문 취소 접수 결과다.
...
// Trade는 v1 주문 상세 응답에 포함된 개별 체결이다.
type Trade struct {
	Market    string `json:"market"`
...
// OrderDetail은 v1 주문 상세와 체결 정보를 보존한다.
type OrderDetail struct {
	...
	CancelingUUID   string          `json:"canceling_uuid"`
	TimeInForce     TimeInForce     `json:"time_in_force"`
	Trades          []Trade         `json:"trades"`
	Raw             json.RawMessage `json:"-"`
}

// OrderSummary는 v2 주문 목록의 주문 상태다.
...
// OrderPage는 v2 주문 목록과 다음 페이지 cursor를 보존한다.
