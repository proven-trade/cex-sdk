...
const (
	defaultDepthSnapshotLimit = 1000
...
var (
	...
	ErrDepthBufferOverflow = errors.New("Binance USD-M depth event buffer overflow")
)

// LocalOrderBookConfig는 Binance USDⓈ-M diff depth와 REST snapshot 동기화 설정이다.
...
func NewLocalOrderBook(config LocalOrderBookConfig) (*LocalOrderBook, error) {
	if config.RESTClient == nil {
		return nil, fmt.Errorf("Binance USD-M local order book REST client is required")
	}
	...
	switch config.SnapshotLimit {
	case 5, 10, 20, 50, 100, 500, 1000:
	default:
		return nil, validationError("local order book snapshot limit must be 5, 10, 20, 50, 100, 500, or 1000")
	}
...
func (book *LocalOrderBook) Run(
	...
	if ctx == nil {
		return fmt.Errorf("Binance USD-M local order book context is nil")
	}
	if market == nil {
		return fmt.Errorf("Binance USD-M local order book market stream is required")
	}
	if handler == nil {
		return fmt.Errorf("Binance USD-M local order book handler is required")
	}
		...
		streamDone <- market.Run(runContext, func(
			...
			if message.Response != nil {
				if message.Response.Error != nil {
					return fmt.Errorf(
						"Binance USD-M depth subscription failed with code %d: %s",
...
func (book *LocalOrderBook) runInputs(
	...
	trySynchronize := func() (bool, error) {
		...
		if first.FirstUpdateID > snapshot.LastUpdateID ||
			first.FinalUpdateID < snapshot.LastUpdateID {
			pendingSnapshot = nil
			scheduleRetry()
			return false, nil
		}
		...
		if err := candidate.applyLevels(first); err != nil {
			return false, err
		}
		for index := 1; index < len(buffer); index++ {
			applied, gap, err := candidate.applyNext(buffer[index].event)
			if err != nil {
				return false, err
			}
			if gap {
	...
	for {
		...
		case input := <-inputs:
			...
			if state != nil {
				applied, gap, err := state.applyNext(input.event)
				if err != nil {
					return err
				}
				if gap {
...
func (book *LocalOrderBook) validateInput(input depthInput) error {
	...
	for _, level := range append(append([]BookLevel(nil), event.Bids...), event.Asks...) {
		if _, _, _, err := parseDepthLevel(level); err != nil {
			return err
		}
	}
	return nil
...
func newLocalDepthState(snapshot OrderBook) (*localDepthState, error) {
	...
	for _, level := range snapshot.Bids {
		if err := applyLocalLevel(state.bids, level); err != nil {
			return nil, err
		}
	}
	for _, level := range snapshot.Asks {
		if err := applyLocalLevel(state.asks, level); err != nil {
			return nil, err
		}
	}
	return state, nil
...
func (state *localDepthState) applyNext(event StreamDepth) (bool, bool, error) {
	if event.FinalUpdateID <= state.lastUpdateID {
		return false, false, nil
	}
	if event.PreviousUpdate != state.lastUpdateID {
		return false, true, nil
	}
	if err := state.applyLevels(event); err != nil {
		return false, false, err
	}
	return true, false, nil
}

func (state *localDepthState) applyLevels(event StreamDepth) error {
	...
	state.transactionTime = event.TransactionTime
	return nil
}

func applyLocalLevel(levels map[string]localDepthLevel, level BookLevel) error {
	key, price, quantity, err := parseDepthLevel(level)
	if err != nil {
		return err
	}
...
func parseDepthLevel(level BookLevel) (string, *big.Rat, string, error) {
	if !decimalPattern.MatchString(level.Price) ||
