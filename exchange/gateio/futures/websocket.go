...
const (
	DefaultWebSocketURL        = "wss://fx-ws.gateio.ws/v4/ws"
	defaultStreamPingInterval  = 30 * time.Second
	defaultStreamPingTimeout   = 10 * time.Second
	maximumStreamSubscriptions = 1000
)

// StreamClientConfig는 Gate.io API v4 무기한 Futures public/private WebSocket 설정이다.
...
// StreamClient는 Gate.io API v4 무기한 Futures WebSocket 세션을 생성한다.
...
// NewStreamClient는 검증된 Gate.io API v4 무기한 Futures WebSocket 클라이언트를 생성한다.
func NewStreamClient(config StreamClientConfig) (*StreamClient, error) {
	if config.Connector == nil {
		return nil, fmt.Errorf("Gate.io Futures stream connector is required")
	}
	...
	webSocketURL, err := validateFuturesWebSocketURL(
		config.WebSocketURL, config.AllowInsecureWebSocket,
	)
	if err != nil {
		return nil, fmt.Errorf("invalid Gate.io Futures WebSocket URL: %w", err)
	}
	config.UserID = strings.TrimSpace(config.UserID)
	if config.UserID != "" && !identifierNumberPattern.MatchString(config.UserID) {
		return nil, validationError("Gate.io Futures stream user ID must contain only digits")
	}
	if config.Now == nil {
		config.Now = time.Now
	}
	...
	if config.PingInterval < 0 || config.PingTimeout < 0 || config.MaxReconnectAttempts < 0 {
		return nil, fmt.Errorf("Gate.io Futures stream durations or reconnect attempts are invalid")
	}
	...
	if config.Credentials != nil {
		...
		if config.CredentialProvider == nil {
			return nil, fmt.Errorf("credential provider is required for private Gate.io Futures streams")
		}
	...
	client := &StreamClient{
		...
		credentialProvider: config.CredentialProvider, userID: config.UserID,
		defaultRouteID: defaultRouteID, webSocketURL: webSocketURL,
		now: config.Now, observer: config.Observer, reconnectPolicy: config.ReconnectPolicy,
...
func (client *StreamClient) PublicStream(
	...
	session, err := client.newStreamSession(request.Settlement, routeID, managed.resubscribe)
	if err != nil {
		return nil, err
	}
...
func (client *StreamClient) PrivateStream(
	...
	if client.credentials == nil || client.credentialProvider == nil {
		return nil, client.streamAuthenticationError(
			errors.New("private Gate.io Futures stream requires credentials"),
		)
	}
	if client.userID == "" {
		return nil, client.streamAuthenticationError(
			errors.New("private Gate.io Futures stream requires numeric user ID"),
		)
	}
...
// Run은 private 주문·체결·잔고·포지션 메시지를 순서대로 해석해 처리기에 전달한다.
...
func (managed *managedStream) run(
	...
	if handler == nil {
		return fmt.Errorf("Gate.io Futures stream handler is required")
	}
...
func (managed *managedStream) change(
	...
	if ctx == nil {
		return fmt.Errorf("Gate.io Futures stream command context is nil")
	}
	managed.commandMu.Lock()
	defer managed.commandMu.Unlock()
	for _, subscription := range subscriptions {
...
func (client *StreamClient) encodeStreamCommand(
	...
	if timestamp <= 0 {
		return nil, "", validationError("Gate.io Futures stream timestamp must be after the Unix epoch")
	}
	...
	if private {
		...
		if len(material.APIKey) == 0 || len(material.SecretKey) == 0 {
			return nil, "", client.streamAuthenticationError(
				errors.New("Gate.io Futures stream API key and HMAC secret are required"),
			)
		}
	...
	payload, err := json.Marshal(struct {
		...
		Payload: client.streamSubscriptionPayload(subscription), Auth: authentication,
	})
	if err != nil {
		return nil, "", fmt.Errorf("encode Gate.io Futures stream command: %w", err)
	}
...
func validateStreamSubscription(subscription StreamSubscription, private bool) error {
	if private {
		...
		case StreamChannelOrders, StreamChannelUserTrades, StreamChannelPositions:
			if subscription.Contract != "!all" {
				if err := validateContract(subscription.Contract); err != nil {
					return err
				}
			}
		case StreamChannelBalances:
			if subscription.Contract != "" {
				return validationError("balance WebSocket subscription does not accept contract")
			}
		default:
	...
	case StreamChannelOrderBookUpdate:
		...
		if subscription.OrderBookLevel != 0 && subscription.OrderBookLevel != 20 &&
			subscription.OrderBookLevel != 50 && subscription.OrderBookLevel != 100 {
			return validationError("WebSocket order book level must be 20, 50, 100 or zero")
		}
	...
	if err := validateStreamContract(subscription); err != nil {
		return err
	}
...
func validateStreamContract(subscription StreamSubscription) error {
	contract := subscription.Contract
	if subscription.Channel == StreamChannelCandles {
		contract = strings.TrimPrefix(contract, "mark_")
		contract = strings.TrimPrefix(contract, "index_")
	}
...
func (client *StreamClient) streamSubscriptionPayload(subscription StreamSubscription) []string {
	switch subscription.Channel {
	case StreamChannelCandles:
		return []string{string(subscription.CandleInterval), subscription.Contract}
	case StreamChannelOrderBookUpdate:
		payload := []string{subscription.Contract, string(subscription.UpdateInterval)}
		if subscription.OrderBookLevel != 0 {
	...
	case StreamChannelOrders, StreamChannelUserTrades, StreamChannelPositions:
		return []string{client.userID, subscription.Contract}
	default:
		return []string{subscription.Contract}
	}
}
...
func (client *StreamClient) resolveStreamRoute(
	...
	if resolved.Timeout != 0 {
		return "", fmt.Errorf("Gate.io Futures stream timeout must be controlled by Run context")
	}
...
func validateFuturesWebSocketURL(value string, allowInsecure bool) (string, error) {
