...
const (
	...
	DefaultOrderQuota     = 10
	DefaultCancelQuota    = 200
	apiPrefix             = "/api/v4"
)

// Config는 Gate.io API v4 Spot REST 클라이언트 설정이다.
type Config struct {
...
// Client는 Gate.io API v4 Spot REST API를 요청별 송신 경로 선택과 함께 제공한다.
type Client struct {
	executor             *commonexchange.Executor
	credentials          *credential.Descriptor
	credentialProvider   credential.Provider
...
// New는 검증된 Gate.io API v4 Spot REST 클라이언트를 생성한다.
func New(config Config) (*Client, error) {
	if config.Executor == nil {
		return nil, fmt.Errorf("Gate.io executor is required")
	}
	...
	if err != nil || parsedBaseURL.Host == "" || parsedBaseURL.User != nil ||
		...
		return nil, fmt.Errorf("invalid Gate.io base URL %q", baseURL)
	}
	if parsedBaseURL.Scheme != "https" && !(config.AllowInsecureHTTP && parsedBaseURL.Scheme == "http") {
		return nil, fmt.Errorf("Gate.io base URL must use HTTPS")
	}
	parsedBaseURL.Path = ""
	...
	if config.RequestTimeout < 0 {
		return nil, fmt.Errorf("Gate.io request timeout cannot be negative")
	}
	...
	if config.PublicQuota < 1 || config.PrivateQuota < 1 || config.OrderQuota < 1 ||
		...
		return nil, fmt.Errorf("Gate.io request quotas must be positive")
	}
	if config.Now == nil {
		config.Now = time.Now
	}
	...
	if config.Credentials != nil {
		...
		if config.CredentialProvider == nil {
			return nil, fmt.Errorf("credential provider is required for private Gate.io requests")
		}
...
func (client *Client) executePrivate(
	...
	if client.credentials == nil || client.credentialProvider == nil {
		return commonexchange.Response{}, &trade.APIError{
			...
			Cause: errors.New("private Gate.io request requires credentials"),
		}
	}
		...
		Build: func(buildContext context.Context) (*http.Request, error) {
			...
			if len(material.APIKey) == 0 || len(material.SecretKey) == 0 {
				return nil, &trade.APIError{
					...
					Cause:     errors.New("Gate.io API key and HMAC secret are required"),
				}
			}
			...
			if timestamp <= 0 {
				return nil, validationError("Gate.io timestamp must be after the Unix epoch")
			}
...
func (client *Client) newRequest(
	...
	if err != nil {
		return nil, fmt.Errorf("create Gate.io request: %w", err)
	}
...
func (client *Client) decodeData(
	...
	if len(trimmed) == 0 || bytes.Equal(trimmed, []byte("null")) {
		return nil, client.decodeBodyError(
			response, operation, errors.New("Gate.io response body is missing"),
		)
	}
...
func (client *Client) errorResponse(
	...
	if err := json.Unmarshal(response.Body, &envelope); err != nil {
		return client.apiError(response, "", "", operation, fmt.Errorf(
			"decode Gate.io JSON error response: %w", err,
		))
	}
...
func (client *Client) decodeBodyError(
	...
	return &trade.APIError{
		...
		Retryable: retryable, HTTPStatus: response.StatusCode,
		Cause: fmt.Errorf("decode Gate.io JSON response: %w", cause),
	}
}
...
func (client *Client) classifyResponse(response commonexchange.Response, operation commonexchange.OperationKind) error {
	...
	_, err := client.decodeData(response, operation, &raw)
	return err
}
