...
func TestNestedMutationErrorObservation(t *testing.T) {
	client := &Client{}
	err := client.classifyResponse(commonexchange.Response{StatusCode: 200, Body: []byte(`{"result":"success","sendStatus":{"status":"insufficientAvailableFunds"}}`)}, commonexchange.OperationMutation)
