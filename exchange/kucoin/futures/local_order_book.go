...
// LocalOrderBookConfig는 KuCoin Pro Futures Increment Best 500 로컬 오더북 설정이다.
...
// LocalOrderBookView는 sequence가 검증된 시점의 정렬된 상위 Futures 호가다.
...
// NewLocalOrderBook은 현재 권장 Pro Futures Increment Best 500 로컬 오더북을 생성한다.
...
// Run은 Pro Futures orderbook stream을 소유하고 snapshot·delta 장부를 handler에 전달한다.
func (book *LocalOrderBook) Run(
	...
	if ctx == nil {
		return fmt.Errorf("KuCoin Futures local order book context is nil")
	}
	if stream == nil {
		return fmt.Errorf("KuCoin Futures Pro order book stream is required")
	}
	if handler == nil {
		return fmt.Errorf("KuCoin Futures local order book handler is required")
	}
	...
	return stream.Run(ctx, func(handlerContext context.Context, message ProOrderBookMessage) error {
		if message.ErrorCode != "" {
			return fmt.Errorf(
				"KuCoin Futures Pro order book subscription failed with code %s: %s",
				message.ErrorCode,
		...
		if message.Result != "" {
			if message.Result != "true" {
				return fmt.Errorf(
					"KuCoin Futures Pro order book subscription was rejected: %s",
					message.Result,
				)
			}
			return nil
...
func pruneLocalOrderBookLevels(
	...
	for _, level := range levels[depth:] {
		price, _ := new(big.Rat).SetString(string(level.Price))
...
func sortedLocalOrderBookLevels(
	...
	for index, level := range levels {
		result[index] = BookLevel{Price: level.priceText, Size: level.sizeText}
	}
	return result
}
