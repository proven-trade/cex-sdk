...
const (
	...
	ratePoolFutures ratePool = "futures"
)
...
func futuresLimit(weight int) endpointLimit {
	return endpointLimit{pool: ratePoolFutures, weight: weight}
}
...
func rateLimitCharges(
	...
	switch limit.pool {
	case ratePoolFutures:
		scope, scopeID, quota = "account", accountID, futuresQuota
	case ratePoolPublic:
	default:
		return rateLimit{}, nil, fmt.Errorf("unsupported KuCoin Futures rate limit pool %q", limit.pool)
	}
	if strings.TrimSpace(scopeID) == "" {
		return rateLimit{}, nil, fmt.Errorf("KuCoin Futures %s rate limit requires scope ID", limit.pool)
	}
	value := rateLimit{
		key:   fmt.Sprintf("kucoin-futures:%s:%s:%s:30seconds", scope, scopeID, limit.pool),
