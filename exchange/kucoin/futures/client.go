...
const (
	DefaultBaseURL        = "https://api-futures.kucoin.com"
	DefaultRequestTimeout = 10 * time.Second
	DefaultAPIKeyVersion  = "2"
	DefaultPublicQuota    = 2000
	DefaultFuturesQuota   = 2000
)

// Config는 KuCoin Classic Futures REST 클라이언트 설정이다.
type Config struct {
...
// New는 KuCoin Classic Futures REST 클라이언트를 생성한다.
func New(config Config) (*Client, error) {
	if config.Executor == nil {
		return nil, fmt.Errorf("KuCoin Futures executor is required")
	}
	...
	if err != nil || parsedBaseURL.Host == "" || parsedBaseURL.User != nil ||
		...
		return nil, fmt.Errorf("invalid KuCoin Futures base URL %q", baseURL)
	}
	if parsedBaseURL.Scheme != "https" && !(config.AllowInsecureHTTP && parsedBaseURL.Scheme == "http") {
		return nil, fmt.Errorf("KuCoin Futures base URL must use HTTPS")
	}
	parsedBaseURL.Path = ""
	...
	if config.RequestTimeout < 0 {
		return nil, fmt.Errorf("KuCoin Futures request timeout cannot be negative")
	}
	if config.APIKeyVersion == "" {
		config.APIKeyVersion = DefaultAPIKeyVersion
	}
	if config.APIKeyVersion != "1" && config.APIKeyVersion != "2" {
		return nil, fmt.Errorf("KuCoin Futures API key version must be 1 or 2")
	}
	if config.PublicQuota == 0 {
		config.PublicQuota = DefaultPublicQuota
	}
	if config.FuturesQuota == 0 {
		config.FuturesQuota = DefaultFuturesQuota
	}
	if config.PublicQuota < 1 || config.FuturesQuota < 1 {
		return nil, fmt.Errorf("KuCoin Futures request quotas must be positive")
	}
	if config.Now == nil {
		config.Now = time.Now
	}
	...
	if config.Credentials != nil {
		...
		if config.CredentialProvider == nil {
			return nil, fmt.Errorf("credential provider is required for private KuCoin Futures requests")
		}
	...
	return &Client{
		...
		futuresQuota: config.FuturesQuota, now: config.Now,
	}, nil
}
...
func (client *Client) executePublic(
	...
	registered, charges, err := rateLimitCharges(
		...
		client.publicQuota, client.futuresQuota,
	)
	if err != nil {
		return commonexchange.Response{}, err
	}
...
func (client *Client) executePrivate(
	...
	if client.credentials == nil || client.credentialProvider == nil {
		return commonexchange.Response{}, &trade.APIError{
			...
			Cause: errors.New("private KuCoin Futures request requires credentials"),
		}
	}
	...
	registered, charges, err := rateLimitCharges(
		...
		limit, client.publicQuota, client.futuresQuota,
	)
	if err != nil {
		return commonexchange.Response{}, err
	}
		...
		Build: func(buildContext context.Context) (*http.Request, error) {
			...
			if len(material.APIKey) == 0 || len(material.SecretKey) == 0 || len(material.Passphrase) == 0 {
				return nil, &trade.APIError{
					...
					Cause: errors.New(
						"KuCoin Futures API key, HMAC secret, and passphrase are required",
					),
				}
			}
			...
			if timestamp == "0" || strings.HasPrefix(timestamp, "-") {
				return nil, validationError("KuCoin Futures timestamp must be after the Unix epoch")
			}
...
func (client *Client) resolveOptions(
	options ...trade.RequestOption,
) (trade.RequestOptions, error) {
...
func (client *Client) newRequest(
	...
	if err != nil {
		return nil, fmt.Errorf("create KuCoin Futures request: %w", err)
	}
...
func (client *Client) responseData(
	...
	if response.StatusCode < http.StatusOK || response.StatusCode >= http.StatusMultipleChoices ||
		...
		if envelope.Code == "" && response.StatusCode >= http.StatusOK &&
			response.StatusCode < http.StatusMultipleChoices {
			return nil, client.decodeBodyError(
				response, operation, errors.New("KuCoin Futures response code is missing"),
			)
		}
		return nil, client.apiError(
			response, envelope.Code, envelope.Message, operation, nil,
		)
	}
...
func (client *Client) decodeData(
	...
	if len(data) == 0 || bytes.Equal(bytes.TrimSpace(data), []byte("null")) {
		return nil, client.decodeBodyError(
			response, operation, errors.New("KuCoin Futures response data is missing"),
		)
	}
...
func (client *Client) decodeBodyError(
	...
	return &trade.APIError{
		...
		Retryable: retryable, HTTPStatus: response.StatusCode,
		Cause: fmt.Errorf("decode KuCoin Futures JSON response: %w", cause),
	}
}
...
func classifyError(
	...
	if status == http.StatusTooManyRequests || code == "1015" || code == "200002" ||
		code == "429000" {
	...
	case "400006", "400007", "40010", "411100":
		return trade.ErrorAuthorization, false
	case "200005", "300003":
		return trade.ErrorInsufficientBalance, false
	case "300002", "300014", "300015", "500000":
		if operation == commonexchange.OperationMutation {
			return trade.ErrorUnknownExecutionState, false
		}
		return trade.ErrorExchangeUnavailable, true
	case "100001", "100003", "200001", "200003", "300000", "300004", "300005",
		"300006", "300007", "300008", "300009", "300010", "300011", "300012",
		"300013", "300016", "300017", "300018", "330005", "330011", "400100":
		return trade.ErrorValidation, false
	}
	lowerMessage := strings.ToLower(message)
	if strings.Contains(lowerMessage, "order") &&
		(strings.Contains(lowerMessage, "not exist") || strings.Contains(lowerMessage, "not found")) {
		return trade.ErrorOrderNotFound, false
	}
	if strings.Contains(lowerMessage, "insufficient balance") || strings.Contains(lowerMessage, "balance not enough") {
		return trade.ErrorInsufficientBalance, false
	}
...
func (client *Client) classifyResponse(response commonexchange.Response, operation commonexchange.OperationKind) error {
	_, err := client.responseData(response, operation)
	return err
}
