...
// ProOrderBookMessage는 Pro public 제어 응답 또는 Futures orderbook 이벤트다.
...
// ProOrderBookStream은 현재 권장 Increment Best 500 Futures 호가 세션이다.
...
// ProOrderBookStream은 token 없는 Pro public endpoint에서 Futures 호가 세션을 생성한다.
...
// Run은 Pro Futures orderbook 메시지를 순서대로 decode해 handler에 전달한다.
func (stream *ProOrderBookStream) Run(ctx context.Context, handler ProOrderBookHandler) error {
	if handler == nil {
		return fmt.Errorf("KuCoin Futures Pro order book stream handler is required")
	}
...
// Close는 Pro public Futures orderbook 세션을 종료한다.
func (stream *ProOrderBookStream) Close() error { return stream.session.Close() }

// Generation은 성공한 Pro public Futures 연결 세대 번호를 반환한다.
func (stream *ProOrderBookStream) Generation() uint64 { return stream.session.Generation() }

// EgressRouteID는 Pro public Futures 연결과 재연결에 고정된 송신 경로를 반환한다.
...
func (stream *ProOrderBookStream) subscribe(
	...
	payload, err := json.Marshal(struct {
		...
		TradeType: "FUTURES", Symbol: stream.symbol, Depth: proOrderBookDepth,
	})
	if err != nil {
		return fmt.Errorf("encode KuCoin Futures Pro order book subscription: %w", err)
	}
	return connection.Write(ctx, corestream.Message{Type: corestream.MessageText, Data: payload})
}

// DecodeProOrderBookMessage는 Pro public Futures orderbook 제어 응답과 이벤트를 분류한다.
func DecodeProOrderBookMessage(message corestream.Message) (ProOrderBookMessage, error) {
	...
	if len(trimmed) == 0 || !json.Valid(trimmed) || trimmed[0] == '[' {
		return ProOrderBookMessage{}, fmt.Errorf("invalid KuCoin Futures Pro order book JSON object")
	}
	var wire proOrderBookWireMessage
	if err := json.Unmarshal(trimmed, &wire); err != nil {
		return ProOrderBookMessage{}, fmt.Errorf("decode KuCoin Futures Pro order book envelope: %w", err)
	}
	id, err := optionalScalarText(wire.ID)
	if err != nil {
		return ProOrderBookMessage{}, fmt.Errorf("decode KuCoin Futures Pro message ID: %w", err)
	}
	result, err := optionalScalarText(wire.Result)
	if err != nil {
		return ProOrderBookMessage{}, fmt.Errorf("decode KuCoin Futures Pro result: %w", err)
	}
	code, err := optionalScalarText(wire.Code)
	if err != nil {
		return ProOrderBookMessage{}, fmt.Errorf("decode KuCoin Futures Pro error code: %w", err)
	}
	...
	if len(bytes.TrimSpace(wire.Data)) > 0 && !bytes.Equal(bytes.TrimSpace(wire.Data), []byte("null")) {
		if err := json.Unmarshal(wire.Data, &decoded.Data); err != nil {
			return ProOrderBookMessage{}, fmt.Errorf("decode KuCoin Futures Pro order book data: %w", err)
		}
	}
	if !strings.EqualFold(decoded.Topic, proOrderBookTopic) || decoded.Depth != proOrderBookDepth ||
		...
		return ProOrderBookMessage{}, fmt.Errorf("unsupported KuCoin Futures Pro order book message")
	}
	if decoded.PublishTime <= 0 || decoded.Data.Symbol == "" {
		return ProOrderBookMessage{}, fmt.Errorf("invalid KuCoin Futures Pro order book event")
	}
	return decoded, nil
}
