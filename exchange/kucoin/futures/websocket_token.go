...
// WebSocketInstanceServer는 KuCoin이 발급한 Futures WebSocket 서버와 heartbeat 규칙이다.
...
// WebSocketToken은 연결 token과 사용 가능한 Futures WebSocket 서버 목록이다.
...
// PublicWebSocketToken은 Classic Futures public WebSocket 연결 token을 발급한다.
...
// PrivateWebSocketToken은 Classic Futures private WebSocket 연결 token을 발급한다.
func (client *Client) PrivateWebSocketToken(
	...
	response, err := client.executePrivate(
		...
		futuresLimit(10), credential.PermissionRead, commonexchange.OperationRead, options...,
	)
	if err != nil {
		return WebSocketToken{}, err
	}
...
func (client *Client) decodeWebSocketToken(
	...
	if wire.Token == "" || len(wire.Servers) == 0 {
		return WebSocketToken{}, client.decodeBodyError(
			...
			errors.New("KuCoin Futures WebSocket token response is incomplete"),
		)
	}
