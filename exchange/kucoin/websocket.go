...
func NewStreamClient(config StreamClientConfig) (*StreamClient, error) {
	...
	if config.RESTClient == nil {
		return nil, fmt.Errorf("KuCoin REST client is required for WebSocket token issuance")
	}
	...
	if config.PingInterval < 0 || config.PingTimeout < 0 ||
		...
		return nil, fmt.Errorf("KuCoin stream durations or reconnect attempts are invalid")
	}
...
func (client *StreamClient) PrivateStream(
	...
	if client.restClient.credentials == nil || client.restClient.credentialProvider == nil {
		return nil, &trade.APIError{
			...
			Cause: errors.New("private KuCoin stream requires credentials"),
		}
	}
...
// Run은 private 주문·잔고 메시지를 순서대로 decode해 handler에 전달한다.
...
func (client *StreamClient) streamDialRequest(
	...
	if err != nil {
		return corestream.DialRequest{}, fmt.Errorf("create KuCoin stream connect ID: %w", err)
	}
	if !identifierPatternForConnectID(connectID) {
		return corestream.DialRequest{}, validationError("invalid KuCoin stream connect ID")
	}
	parsed, err := url.Parse(server.Endpoint)
	if err != nil {
		return corestream.DialRequest{}, fmt.Errorf("parse KuCoin stream endpoint: %w", err)
	}
	query := parsed.Query()
...
func (client *StreamClient) selectInstanceServer(
	...
	for _, server := range servers {
		...
		if client.pingInterval >= time.Duration(server.PingInterval)*time.Millisecond {
			return WebSocketInstanceServer{}, fmt.Errorf("KuCoin stream ping interval must be shorter than server interval")
		}
		if client.pingTimeout > time.Duration(server.PingTimeout)*time.Millisecond {
			return WebSocketInstanceServer{}, fmt.Errorf("KuCoin stream ping timeout exceeds server timeout")
		}
		return server, nil
	}
	return WebSocketInstanceServer{}, fmt.Errorf("KuCoin WebSocket token has no usable instance server")
}
...
func (managed *managedStream) run(
	...
	if handler == nil {
		return fmt.Errorf("KuCoin stream handler is required")
	}
...
func (managed *managedStream) change(
	...
	if ctx == nil {
		return fmt.Errorf("KuCoin stream command context is nil")
	}
	managed.commandMu.Lock()
	defer managed.commandMu.Unlock()
	for _, subscription := range subscriptions {
...
func encodeStreamCommand(
	...
	if err != nil {
		return nil, fmt.Errorf("encode KuCoin stream command: %w", err)
	}
	return payload, nil
}
...
func validateStreamSubscription(subscription StreamSubscription, private bool) error {
	if private {
		if subscription.Channel != StreamChannelOrders && subscription.Channel != StreamChannelBalance {
		...
		if subscription.Symbol != "" || subscription.Interval != "" {
			return validationError("private WebSocket subscription does not accept symbol or interval")
		}
		return nil
	}
...
func streamTopic(subscription StreamSubscription) string {
	...
	case StreamChannelTicker:
		return "/market/ticker:" + subscription.Symbol
	case StreamChannelLevel2:
		return "/market/level2:" + subscription.Symbol
	case StreamChannelOrderBook5:
		return "/spotMarket/level2Depth5:" + subscription.Symbol
	case StreamChannelOrderBook50:
		return "/spotMarket/level2Depth50:" + subscription.Symbol
	case StreamChannelCandles:
		return "/market/candles:" + subscription.Symbol + "_" + string(subscription.Interval)
	case StreamChannelTrade:
		return "/market/match:" + subscription.Symbol
...
func (client *StreamClient) resolveStreamRoute(
	...
	if resolved.Timeout != 0 {
		return "", fmt.Errorf("KuCoin stream timeout must be controlled by Run context")
	}
...
func (connection *kucoinConnection) Ping(ctx context.Context) error {
	...
	if err != nil {
		return fmt.Errorf("encode KuCoin stream ping: %w", err)
	}
