...
func TestDecodeProOrderBookMessage(t *testing.T) {
	...
	message, err := DecodeProOrderBookMessage(corestream.Message{Data: []byte(
		`{"T":"obu.SPOT","dp":"increment@10ms","t":"snapshot","P":1725000000000000000,"d":{"O":100,"C":100,"M":1724999999999999999,"s":"BTC-USDT","a":[["101","2"]],"b":[["100","1"]]}}`,
	)})
	if err != nil {
	...
	if message.Topic != proOrderBookTopic || message.Depth != proOrderBookDepth ||
		...
		message.Data.Symbol != "BTC-USDT" || len(message.Data.Bids) != 1 ||
		message.Data.Bids[0] != (BookLevel{Price: "100", Size: "1"}) || len(message.Raw) == 0 {
...
func TestDecodeProOrderBookMessageRejectsInvalidPayloads(t *testing.T) {
	...
	payloads := []string{
		``,
		`[]`,
		`{"T":"obu.FUTURES","dp":"increment@10ms","t":"snapshot","P":1,"d":{"s":"BTC-USDT"}}`,
		`{"T":"obu.SPOT","dp":"increment@100ms","t":"snapshot","P":1,"d":{"s":"BTC-USDT"}}`,
		`{"T":"obu.SPOT","dp":"increment@10ms","t":"replace","P":1,"d":{"s":"BTC-USDT"}}`,
		`{"T":"obu.SPOT","dp":"increment@10ms","t":"snapshot","P":0,"d":{"s":"BTC-USDT"}}`,
		`{"T":"obu.SPOT","dp":"increment@10ms","t":"snapshot","P":1,"d":{"s":""}}`,
	}
...
func TestProOrderBookStreamReconnectsOnSelectedRouteWithoutToken(t *testing.T) {
	...
	second.reads <- kucoinWebSocketReadResult{message: corestream.Message{Data: []byte(
		`{"T":"obu.SPOT","dp":"increment@10ms","t":"snapshot","P":2000,"d":{"O":200,"C":200,"M":1999,"s":"BTC-USDT","a":[["101","1"]],"b":[["100","1"]]}}`,
	)}}
	select {
	case runErr := <-done:
	...
	case <-time.After(time.Second):
		t.Fatal("Pro order book stream did not finish")
	}
...
func TestProOrderBookStreamValidation(t *testing.T) {
	...
	if _, err := client.ProOrderBookStream("BTCBTC"); err == nil {
		t.Fatal("ProOrderBookStream() invalid symbol error = nil")
	}
	if _, err := client.ProOrderBookStream("BTC-USDT", trade.WithTimeout(time.Second)); err == nil || !strings.Contains(err.Error(), "Run context") {
		t.Fatalf("ProOrderBookStream() timeout error = %v", err)
	}
	stream, err := client.ProOrderBookStream("BTC-USDT")
	if err != nil {
		t.Fatalf("ProOrderBookStream() error = %v", err)
	}
...
func TestValidateProPublicWebSocketURL(t *testing.T) {
	...
	tests := []struct {
		...
		{raw: "wss://x-push-spot.kucoin.com/", want: "wss://x-push-spot.kucoin.com"},
	...
	for _, raw := range []string{
		"http://x-push-spot.kucoin.com",
		"ws://x-push-spot.kucoin.com",
		"wss://user@x-push-spot.kucoin.com",
		"wss://x-push-spot.kucoin.com?token=secret",
		"wss://x-push-spot.kucoin.com/#fragment",
	} {
...
func assertProOrderBookSubscription(t *testing.T, payload []byte, symbol string) {
	...
	if err := json.Unmarshal(payload, &subscription); err != nil {
		t.Fatalf("decode Pro order book subscription: %v", err)
	}
	if subscription.ID == "" || subscription.Action != "SUBSCRIBE" ||
		subscription.Channel != "obu" || subscription.TradeType != "SPOT" ||
		subscription.Symbol != symbol || subscription.Depth != proOrderBookDepth ||
		subscription.RPIFilter != 0 {
		t.Fatalf("Pro order book subscription = %+v", subscription)
	}
}
