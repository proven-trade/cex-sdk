...
// LocalOrderBookConfig는 업비트 Spot 전체 snapshot 오더북 설정이다.
...
// LocalOrderBook은 업비트의 완전한 SNAPSHOT·REALTIME 호가를 안전한 view로 변환한다.
...
func (book *LocalOrderBook) Run(
	...
	if ctx == nil {
		return fmt.Errorf("Upbit local order book context is nil")
	}
	if public == nil {
		return fmt.Errorf("Upbit local order book public stream is required")
	}
	if handler == nil {
		return fmt.Errorf("Upbit local order book handler is required")
	}
	...
	return public.Run(ctx, func(_ context.Context, message StreamMessage) error {
		if message.Error != nil {
			return fmt.Errorf(
				"Upbit order book subscription failed with %s: %s",
				message.Error.Name,
...
func validateLocalOrderBookSnapshot(market string, event StreamOrderBook) error {
	...
	if len(event.OrderBook) < 1 || len(event.OrderBook) > 30 {
		return validationError("local order book must contain between 1 and 30 units")
	}
	if _, err := nonnegativeStreamDecimal("total ask size", event.TotalAskSize); err != nil {
		return err
	}
	if _, err := nonnegativeStreamDecimal("total bid size", event.TotalBidSize); err != nil {
		return err
	}
	if event.Level != "" {
		if _, err := nonnegativeStreamDecimal("orderbook level", event.Level); err != nil {
			return err
		}
	}
	...
	for _, unit := range event.OrderBook {
		...
		if _, err := nonnegativeStreamDecimal("ask size", unit.AskSize); err != nil {
			return err
		}
		if _, err := nonnegativeStreamDecimal("bid size", unit.BidSize); err != nil {
			return err
		}
...
func positiveLocalOrderBookDecimal(name string, value Decimal) (*big.Rat, error) {
	parsed, err := nonnegativeStreamDecimal(name, value)
