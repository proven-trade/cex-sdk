...
// QueryHash는 업비트 인증에 사용하는 SHA-512 쿼리 해시를 반환한다.
func QueryHash(query string) string {
	digest := sha512.Sum512([]byte(query))
	return hex.EncodeToString(digest[:])
}

// SignJWT는 Access Key와 Secret Key로 업비트 HS512 JWT를 만든다.
func SignJWT(accessKey, secretKey []byte, nonce, query string) (string, error) {
	if len(accessKey) == 0 || len(secretKey) == 0 {
		return "", fmt.Errorf("Upbit access key and secret key are required")
	}
	if nonce == "" {
		return "", fmt.Errorf("Upbit JWT nonce is required")
	}
	payload := struct {
	...
	if err != nil {
		return "", fmt.Errorf("encode Upbit JWT payload: %w", err)
	}
...
func randomNonce() (string, error) {
	...
	if _, err := rand.Read(value); err != nil {
		return "", fmt.Errorf("generate Upbit JWT nonce: %w", err)
	}
