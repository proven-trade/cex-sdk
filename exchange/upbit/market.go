...
// Markets는 업비트가 지원하는 전체 마켓을 조회한다.
func (client *Client) Markets(
	ctx context.Context,
	request MarketsRequest,
	options ...trade.RequestOption,
) ([]Market, error) {
	response, err := client.executePublic(ctx, "/v1/market/all", request.parameters(), marketRate, options...)
	if err != nil {
		return nil, err
	}
...
func (client *Client) Tickers(
	...
	response, err := client.executePublic(ctx, "/v1/ticker", request.parameters(), tickerRate, options...)
	if err != nil {
		return nil, err
	}
...
func (client *Client) OrderBooks(
	...
	response, err := client.executePublic(ctx, "/v1/orderbook", request.parameters(), orderBookRate, options...)
	if err != nil {
		return nil, err
	}
...
func (client *Client) RecentTrades(
	...
	response, err := client.executePublic(ctx, "/v1/trades/ticks", request.parameters(), tradeRate, options...)
	if err != nil {
		return nil, err
	}
...
func (client *Client) MinuteCandles(
	...
	response, err := client.executePublic(ctx, path, request.parameters(), candleRate, options...)
	if err != nil {
		return nil, err
	}
...
func decodeItems[T any](
	...
	for index, rawItem := range rawItems {
		if err := decodeJSON(rawItem, &items[index]); err != nil {
			return nil, client.decodeBodyError(response, operation, fmt.Errorf("decode Upbit response item: %w", err))
		}
		if setRaw != nil {
